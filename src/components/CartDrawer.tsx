  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button variant="outline" size="icon" className="relative border-border/50 hover:border-primary/50 bg-background/50 backdrop-blur-sm">
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full bg-background/95 backdrop-blur-xl border-border">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle className="font-space gradient-text">Shopping Cart</SheetTitle>
          <SheetDescription>
          ...
          {items.length === 0 ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1">Browse our merchandise to get started</p>
              </div>
            </div>
          ) : (
            <>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-3 rounded-xl bg-muted/30 border border-border/50">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-medium font-space truncate text-foreground">{item.product.node.title}</h4>
                      ...
                      <div className="flex flex-col items-end gap-2 flex-shrink-0">
                        <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive hover:text-destructive" onClick={() => removeItem(item.variantId)}>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t border-border bg-background">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold gradient-text">{items[0]?.price.currencyCode || '$'} {totalPrice.toFixed(2)}</span>
                </div>
